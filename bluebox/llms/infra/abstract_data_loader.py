...
Provides common patterns for data loaders including:
...
class AbstractDataLoader(ABC, Generic[EntryT, StatsT]):
    """
    Abstract base class for data loaders.
    ...
    def search_by_regex(
        ...
        def _search_worker() -> None:
            ...
            for entry in self._entries:
                ...
                try:
                    for match in compiled.finditer(string=content, timeout=per_entry_timeout):
                        if stop_event.is_set():
                            timed_out = True
                            break
