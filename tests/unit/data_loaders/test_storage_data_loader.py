"""
tests/unit/data_loaders/test_storage_data_loader.py

Comprehensive unit tests for StorageDataLoader and related classes.
"""
...
from bluebox.llms.data_loaders.storage_data_loader import (
    StorageDataLoader,
    StorageStats,
)
...
@pytest.fixture
def basic_store(storage_events_dir: Path) -> StorageDataLoader:
    """StorageDataLoader loaded from basic test data."""
    return StorageDataLoader(str(storage_events_dir / "storage_basic.jsonl"))


@pytest.fixture
def search_store(storage_events_dir: Path) -> StorageDataLoader:
    """StorageDataLoader loaded from search test data."""
    return StorageDataLoader(str(storage_events_dir / "storage_search.jsonl"))


@pytest.fixture
def origins_store(storage_events_dir: Path) -> StorageDataLoader:
    """StorageDataLoader loaded from origins test data."""
    return StorageDataLoader(str(storage_events_dir / "storage_origins.jsonl"))


@pytest.fixture
def malformed_store(storage_events_dir: Path) -> StorageDataLoader:
    """StorageDataLoader loaded from malformed test data (should skip bad lines)."""
    return StorageDataLoader(str(storage_events_dir / "storage_malformed.jsonl"))
...
# --- StorageDataLoader Initialization Tests ---


class TestStorageDataLoaderInit:
    """Tests for StorageDataLoader initialization."""

    def test_init_basic_file(self, basic_store: StorageDataLoader) -> None:
    ...
    def test_init_file_not_found(self, storage_events_dir: Path) -> None:
        ...
        with pytest.raises(FileNotFoundError):
            StorageDataLoader(str(storage_events_dir / "nonexistent.jsonl"))

    def test_init_empty_file(self, storage_events_dir: Path) -> None:
        """Initialize from empty file produces empty store."""
        store = StorageDataLoader(str(storage_events_dir / "storage_empty.jsonl"))
        assert len(store.entries) == 0

    def test_init_malformed_skips_bad_lines(self, malformed_store: StorageDataLoader) -> None:
...
class TestStorageDataLoaderProperties:
    """Tests for StorageDataLoader properties."""

    def test_entries_returns_list(self, basic_store: StorageDataLoader) -> None:
        """entries property returns list of StorageEvent."""
        entries = basic_store.entries
        assert isinstance(entries, list)
        assert len(entries) > 0

    def test_stats_returns_storage_stats(self, basic_store: StorageDataLoader) -> None:
    ...
    def test_stats_counts_cookie_events(self, basic_store: StorageDataLoader) -> None:
        """Stats correctly counts cookie events."""
        stats = basic_store.stats
        # We have initialCookies and cookieChange events
        assert stats.cookie_events == 2

    def test_stats_counts_local_storage_events(self, basic_store: StorageDataLoader) -> None:
    ...
    def test_stats_counts_session_storage_events(self, basic_store: StorageDataLoader) -> None:
        """Stats correctly counts sessionStorage events."""
        stats = basic_store.stats
        # Two sessionStorageItemAdded events
        assert stats.session_storage_events == 2

    def test_stats_counts_indexed_db_events(self, basic_store: StorageDataLoader) -> None:
        """Stats correctly counts IndexedDB events."""
        stats = basic_store.stats
        assert stats.indexed_db_events == 1

    def test_stats_counts_unique_origins(self, basic_store: StorageDataLoader) -> None:
        """Stats correctly counts unique origins."""
        stats = basic_store.stats
        # https://example.com and https://api.example.com
        assert stats.unique_origins == 2

    def test_stats_counts_unique_keys(self, basic_store: StorageDataLoader) -> None:
...
class TestEntryRetrieval:
    ...
    def test_get_entry_found(self, basic_store: StorageDataLoader) -> None:
        """Get entry by valid index."""
        entry = basic_store.get_entry(0)
        assert entry is not None
        assert entry.type == "initialCookies"

    def test_get_entry_not_found(self, basic_store: StorageDataLoader) -> None:
        """Return None for non-existent index."""
        entry = basic_store.get_entry(999)
        assert entry is None

    def test_get_entries_by_origin_found(self, origins_store: StorageDataLoader) -> None:
    ...
    def test_get_entries_by_origin_not_found(self, origins_store: StorageDataLoader) -> None:
        """Return empty list when origin not found."""
        results = origins_store.get_entries_by_origin("https://nonexistent.com")
        assert results == []

    def test_get_entries_by_key_found(self, origins_store: StorageDataLoader) -> None:
    ...
    def test_get_entries_by_key_not_found(self, origins_store: StorageDataLoader) -> None:
        """Return empty list when key not found."""
        results = origins_store.get_entries_by_key("nonexistent_key")
        assert results == []

    def test_get_entries_by_origin_and_key(self, origins_store: StorageDataLoader) -> None:
    ...
    def test_get_entries_by_origin_and_key_no_match(self, origins_store: StorageDataLoader) -> None:
...
class TestSearchValues:
    ...
    def test_search_finds_value_in_value_field(self, search_store: StorageDataLoader) -> None:
    ...
    def test_search_finds_value_in_new_value_field(self, search_store: StorageDataLoader) -> None:
    ...
    def test_search_finds_value_in_old_value_field(self, search_store: StorageDataLoader) -> None:
    ...
    def test_search_finds_value_in_added_field(self, search_store: StorageDataLoader) -> None:
    ...
    def test_search_finds_value_in_modified_field(self, search_store: StorageDataLoader) -> None:
    ...
    def test_search_multiple_locations_same_entry(self, search_store: StorageDataLoader) -> None:
    ...
    def test_search_case_insensitive_default(self, search_store: StorageDataLoader) -> None:
        """Search is case-insensitive by default."""
        results_lower = search_store.search_values("alice")
        results_upper = search_store.search_values("ALICE")
        assert len(results_lower) == len(results_upper)

    def test_search_case_sensitive(self, search_store: StorageDataLoader) -> None:
    ...
    def test_search_empty_value(self, search_store: StorageDataLoader) -> None:
        """Return empty list for empty search value."""
        results = search_store.search_values("")
        assert results == []

    def test_search_no_matches(self, search_store: StorageDataLoader) -> None:
        """Return empty list when value not found."""
        results = search_store.search_values("xyznonexistent123")
        assert results == []

    def test_search_result_structure(self, search_store: StorageDataLoader) -> None:
    ...
    def test_search_returns_correct_origin(self, search_store: StorageDataLoader) -> None:
    ...
    def test_search_returns_correct_key(self, search_store: StorageDataLoader) -> None:
...
class TestFeatureDetection:
    ...
    def test_mixed_storage_types(self, basic_store: StorageDataLoader) -> None:
