"""
tests/unit/test_window_property_data_store.py

Comprehensive unit tests for WindowPropertyDataStore and related classes.
"""
...
from bluebox.llms.infra.window_property_data_store import (
...
@pytest.fixture
def basic_store(window_property_events_dir: Path) -> WindowPropertyDataStore:
    """WindowPropertyDataStore loaded from basic test data."""
    return WindowPropertyDataStore(str(window_property_events_dir / "window_basic.jsonl"))


@pytest.fixture
def search_store(window_property_events_dir: Path) -> WindowPropertyDataStore:
    """WindowPropertyDataStore loaded from search test data."""
    return WindowPropertyDataStore(str(window_property_events_dir / "window_search.jsonl"))


@pytest.fixture
def paths_store(window_property_events_dir: Path) -> WindowPropertyDataStore:
    """WindowPropertyDataStore loaded from paths test data."""
    return WindowPropertyDataStore(str(window_property_events_dir / "window_paths.jsonl"))


@pytest.fixture
def malformed_store(window_property_events_dir: Path) -> WindowPropertyDataStore:
    """WindowPropertyDataStore loaded from malformed test data (should skip bad lines)."""
    return WindowPropertyDataStore(str(window_property_events_dir / "window_malformed.jsonl"))
...
# --- WindowPropertyDataStore Initialization Tests ---


class TestWindowPropertyDataStoreInit:
    """Tests for WindowPropertyDataStore initialization."""

    def test_init_basic_file(self, basic_store: WindowPropertyDataStore) -> None:
    ...
    def test_init_empty_file(self, window_property_events_dir: Path) -> None:
        ...
        store = WindowPropertyDataStore(str(window_property_events_dir / "window_empty.jsonl"))
        assert len(store.entries) == 0

    def test_init_malformed_skips_bad_lines(self, malformed_store: WindowPropertyDataStore) -> None:
...
class TestWindowPropertyDataStoreProperties:
    """Tests for WindowPropertyDataStore properties."""

    def test_entries_returns_list(self, basic_store: WindowPropertyDataStore) -> None:
        """entries property returns list of WindowPropertyEvent."""
        entries = basic_store.entries
        assert isinstance(entries, list)
        assert len(entries) > 0

    def test_stats_returns_window_property_stats(self, basic_store: WindowPropertyDataStore) -> None:
    ...
    def test_stats_counts_total_changes(self, basic_store: WindowPropertyDataStore) -> None:
    ...
    def test_stats_counts_added_changes(self, basic_store: WindowPropertyDataStore) -> None:
        """Stats correctly counts added changes."""
        stats = basic_store.stats
        # Most changes in basic_store are "added"
        assert stats.changes_added >= 8

    def test_stats_counts_changed_changes(self, basic_store: WindowPropertyDataStore) -> None:
        """Stats correctly counts changed changes."""
        stats = basic_store.stats
        # scrollY changed, innerWidth changed
        assert stats.changes_changed >= 2

    def test_stats_counts_deleted_changes(self, basic_store: WindowPropertyDataStore) -> None:
        """Stats correctly counts deleted changes."""
        stats = basic_store.stats
        # __userData was deleted
        assert stats.changes_deleted >= 1

    def test_stats_counts_unique_urls(self, basic_store: WindowPropertyDataStore) -> None:
    ...
    def test_stats_counts_unique_property_paths(self, basic_store: WindowPropertyDataStore) -> None:
...
class TestEntryRetrieval:
    ...
    def test_get_entry_found(self, basic_store: WindowPropertyDataStore) -> None:
        """Get entry by valid index."""
        entry = basic_store.get_entry(0)
        assert entry is not None
        assert entry.url == "https://example.com/"

    def test_get_entry_not_found(self, basic_store: WindowPropertyDataStore) -> None:
        """Return None for non-existent index."""
        entry = basic_store.get_entry(999)
        assert entry is None

    def test_get_entry_has_changes(self, basic_store: WindowPropertyDataStore) -> None:
...
class TestGetChangesByPath:
    ...
    def test_exact_path_match(self, paths_store: WindowPropertyDataStore) -> None:
    ...
    def test_exact_path_no_match(self, paths_store: WindowPropertyDataStore) -> None:
        """Return empty list when exact path not found."""
        results = paths_store.get_changes_by_path("nonexistent.path", exact=True)
        assert results == []

    def test_substring_path_match(self, paths_store: WindowPropertyDataStore) -> None:
    ...
    def test_substring_path_case_insensitive(self, paths_store: WindowPropertyDataStore) -> None:
    ...
    def test_result_structure(self, paths_store: WindowPropertyDataStore) -> None:
    ...
    def test_tracks_change_types(self, paths_store: WindowPropertyDataStore) -> None:
    ...
    def test_includes_url(self, paths_store: WindowPropertyDataStore) -> None:
...
class TestSearchValues:
    ...
    def test_search_finds_string_value(self, search_store: WindowPropertyDataStore) -> None:
        """Find changes with matching string value."""
        results = search_store.search_values("Alice Johnson")
        assert len(results) > 0

    def test_search_finds_nested_value(self, search_store: WindowPropertyDataStore) -> None:
    ...
    def test_search_finds_numeric_value(self, search_store: WindowPropertyDataStore) -> None:
        """Find changes with matching numeric value."""
        results = search_store.search_values("29.99")
        assert len(results) > 0

    def test_search_case_insensitive_default(self, search_store: WindowPropertyDataStore) -> None:
        """Search is case-insensitive by default."""
        results_lower = search_store.search_values("alice")
        results_upper = search_store.search_values("ALICE")
        assert len(results_lower) == len(results_upper)

    def test_search_case_sensitive(self, search_store: WindowPropertyDataStore) -> None:
    ...
    def test_search_empty_value(self, search_store: WindowPropertyDataStore) -> None:
        """Return empty list for empty search value."""
        results = search_store.search_values("")
        assert results == []

    def test_search_no_matches(self, search_store: WindowPropertyDataStore) -> None:
        """Return empty list when value not found."""
        results = search_store.search_values("xyznonexistent123")
        assert results == []

    def test_search_skips_null_values(self, search_store: WindowPropertyDataStore) -> None:
    ...
    def test_search_result_structure(self, search_store: WindowPropertyDataStore) -> None:
    ...
    def test_search_returns_correct_url(self, search_store: WindowPropertyDataStore) -> None:
    ...
    def test_search_returns_correct_path(self, search_store: WindowPropertyDataStore) -> None:
        """Search results include correct path."""
        results = search_store.search_values("TRACK-ABC-123")
        assert len(results) == 1
        assert results[0]["path"] == "__trackingId"

    def test_search_returns_change_type(self, search_store: WindowPropertyDataStore) -> None:
...
class TestFeatureDetection:
    ...
    def test_mixed_change_types(self, basic_store: WindowPropertyDataStore) -> None:
    ...
    def test_changes_sum_equals_total(self, basic_store: WindowPropertyDataStore) -> None:
