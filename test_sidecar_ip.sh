#!/bin/bash
# Timing test: IP check via sidecar with rotating proxies
#
# Prerequisites:
...
echo "IP Address Check via SIDECAR — 10 proxies"
echo "------------------------------------------------------------------------"
printf "%-5s %-3s %-16s %7s  %s\n" "Run" "OK" "IP" "Time" "Proxy"
echo "------------------------------------------------------------------------"
...
for entry in "${PROXIES[@]}"; do
  ...
  status="❌"
  [ -n "$ip" ] && status="✅" && ((passed++)) || { ip="FAILED"; ((failed++)); }

  printf "%-5s %s  %-16s %6ss  %s\n" "$i" "$status" "$ip" "$elapsed" "$label"
